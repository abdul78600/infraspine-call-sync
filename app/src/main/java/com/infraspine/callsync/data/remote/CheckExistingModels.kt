 ...
 *
 * Each record carries a [CallLogCheckItem.clientRef] / [RecordingCheckItem.clientRef]
 * — an opaque client-side identifier — which the server echoes back in
 * [CallLogExistingCheckResponse.existing]/[missing] (and the recording equivalents).
 * This avoids needing the client and server to agree on a composite dedup-key
 * format: the server matches using its own dedup logic and just tells the
 * client which of *its* records it already has.
 */
