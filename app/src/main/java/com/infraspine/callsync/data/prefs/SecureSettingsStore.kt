...
class SecureSettingsStore(context: Context) {
    ...
    /** Stored encrypted — used only for silent auto re-login when token expires. */
    var savedPassword: String?
        get() = prefs.getString(KEY_SAVED_PASSWORD, null)?.takeIf { it.isNotBlank() }
        set(value) = prefs.edit { putString(KEY_SAVED_PASSWORD, value) }
